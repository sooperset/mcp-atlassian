...
def ensure_dict_from_json(v: Any) -> dict[str, Any]:
    ...
    if isinstance(v, str):
        try:
            ...
            print(
                f"Validator: Failed to parse as JSON dict ({e}), keeping original string: '{v}'"
            )
            raise ValueError(
                "Input is not a valid JSON dictionary string"
            ) from e  # Raise to keep original
    elif isinstance(v, dict):
        print(f"Validator: Input is already a dict: {v}")
        return v  # Already a dict, pass through
    else:
        raise ValueError(
            f"Input must be a dict or a valid JSON string representing a dict, got {type(v)}"
        )
    ...
    async with Client(mcp) as client:
        ...
        try:
            result_raw = await client.call_tool(
                "process_raw_string", {"data_str": json_string_input}
            )
        ...
        try:
            result_dict = await client.call_tool(
                "process_dict_direct", {"data_dict": json_string_input}
            )
            print(f"Result (process_dict_direct): {result_dict[0].text}")
        except Exception as e:
            print(
                f"Error (process_dict_direct): {e}"
            )  # Expecting potential error if auto-parsing fails

        # 3. Pydantic Model Argument (FastMCP might auto-parse JSON string here)
        try:
            result_model = await client.call_tool(
                "process_pydantic_model", {"data_model": json_string_input}
            )
            print(f"Result (process_pydantic_model): {result_model[0].text}")
        except Exception as e:
            print(
                f"Error (process_pydantic_model): {e}"
            )  # Expecting potential error if auto-parsing fails

        # 4. Annotated Dict with Validator
        try:
            result_validated = await client.call_tool(
                "process_validated_dict", {"data": json_string_input}
            )
            print(f"Result (process_validated_dict): {result_validated[0].text}")
        except Exception as e:
            print(
                f"Error (process_validated_dict): {e}"
            )  # Should succeed if validator works
        ...
        try:
            result_raw_dict = await client.call_tool(
                "process_raw_string", {"data_str": dict_input}
            )
        ...
        try:
            result_dict_dict = await client.call_tool(
                "process_dict_direct", {"data_dict": dict_input}
            )
        ...
        try:
            result_model_dict = await client.call_tool(
                "process_pydantic_model", {"data_model": dict_input}
            )
            print(
                f"Result (process_pydantic_model with dict): {result_model_dict[0].text}"
            )
        except Exception as e:
        ...
        try:
            result_validated_dict = await client.call_tool(
                "process_validated_dict", {"data": dict_input}
            )
            print(
                f"Result (process_validated_dict with dict): {result_validated_dict[0].text}"
            )
        except Exception as e:
        ...
        try:
            result_list_direct = await client.call_tool(
                "process_list_direct", {"data_list": list_input_ok}
            )
            print(
                f"Result (process_list_direct with list): {result_list_direct[0].text}"
            )
        except Exception as e:
        ...
        try:
            result_list_json = await client.call_tool(
                "process_list_direct", {"data_list": list_input_json_str_ok}
            )
            print(
                f"Result (process_list_direct with JSON string): {result_list_json[0].text}"
            )
        except Exception as e:
        ...
        try:
            result_list_json_bad = await client.call_tool(
                "process_list_direct", {"data_list": list_input_json_str_bad}
            )
            print(
                f"Result (process_list_direct with bad JSON string): {result_list_json_bad[0].text}"
            )
        except Exception as e:
            print(
                f"Error (process_list_direct with bad JSON string): {e}"
            )  # Expect error

        # 8. Annotated List Argument (Direct Python list)
        try:
            result_annotated_list = await client.call_tool(
                "process_annotated_list", {"data_list": list_input_ok}
            )
            print(
                f"Result (process_annotated_list with list): {result_annotated_list[0].text}"
            )
        except Exception as e:
        ...
        try:
            result_annotated_json = await client.call_tool(
                "process_annotated_list", {"data_list": list_input_json_str_ok}
            )
            print(
                f"Result (process_annotated_list with JSON string): {result_annotated_json[0].text}"
            )
        except Exception as e:

...
class MarkdownSyncConfig:
    ...
    def from_env(cls) -> "MarkdownSyncConfig":
        ...
        return cls(
            enabled=os.getenv("ATLASSIAN_MARKDOWN_SYNC_ENABLED", "false").lower()
            == "true",
            ...
            preserve_hierarchy=os.getenv(
                "ATLASSIAN_MARKDOWN_PRESERVE_HIERARCHY", "true"
            ).lower()
            == "true",
            auto_create_pages=os.getenv(
            ...
            match_threshold=float(
                os.getenv("ATLASSIAN_MARKDOWN_MATCH_THRESHOLD", "85.0")
            ),
            preserve_formatting=os.getenv(
                "ATLASSIAN_MARKDOWN_PRESERVE_FORMAT", "true"
            ).lower()
            == "true",
            convert_links=os.getenv("ATLASSIAN_MARKDOWN_CONVERT_LINKS", "true").lower()
            == "true",
            handle_attachments=os.getenv(

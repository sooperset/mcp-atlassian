...
from ...utils.decorators import convert_empty_defaults_to_none
...
async def sync_markdown_to_page(
    ...
    file_path: Annotated[
        ...
        Field(
            description=(
                ...
                "Can be relative (e.g., './docs/project-overview.md') or absolute path. "
    ...
    logger.info(
        f"Starting confluence_sync_markdown_to_page: file_path={file_path}, space_key={space_key}, parent_id={parent_id}, sync_mode={sync_mode}, conflict_strategy={conflict_strategy}, dry_run={dry_run}"
    )
    ...
    try:
        ...
        if not os.path.isabs(file_path):
            # Make path relative to current workspace directory
            current_workspace = os.getcwd()
            resolved_file_path = os.path.join(current_workspace, file_path)
            logger.debug(
                f"Converting relative path {file_path} to absolute: {resolved_file_path}"
            )
        else:
            resolved_file_path = file_path
            logger.debug(f"Using absolute path: {resolved_file_path}")

        # Check if file exists
        if not os.path.exists(resolved_file_path):
            logger.error(f"Markdown file not found: {resolved_file_path}")
            error_response = {
                "success": False,
                "message": f"Markdown file not found: {file_path}",
                "page_id": None,
                "details": {
        ...
        logger.debug(f"File exists at: {resolved_file_path}")
        ...
        logger.debug(
            f"Converting parameters to enums: sync_mode={sync_mode}, conflict_strategy={conflict_strategy}"
        )
        ...
        # Perform the sync
        logger.debug(
            f"About to call sync_markdown_to_page with parameters: file_path={resolved_file_path}, space_key={space_key}, parent_id={parent_id if parent_id else None}, sync_mode={sync_mode_enum}, conflict_strategy={conflict_strategy_enum}, dry_run={dry_run}"
        )
        ...
        logger.debug(
            f"Sync operation completed with result: success={result.success}, message={result.message}, page_id={result.page_id}"
        )
...
async def sync_markdown_batch(
    ...
    # Log function entry with parameters
    logger.info(
        f"Starting confluence_sync_markdown_batch: files={files}, space_key={space_key}, sync_mode={sync_mode}, conflict_strategy={conflict_strategy}, preserve_hierarchy={preserve_hierarchy}, dry_run={dry_run}"
    )
    ...
    try:
        ...
        for file_pattern in files:
            if "*" in file_pattern or "?" in file_pattern:
                ...
                # Check if the pattern is absolute, if not make it relative to current workspace
                if not os.path.isabs(file_pattern):
                    # Make pattern relative to current workspace directory
                    current_workspace = os.getcwd()
                    absolute_pattern = os.path.join(current_workspace, file_pattern)
                    logger.debug(
                        f"Converting relative pattern {file_pattern} to absolute: {absolute_pattern}"
                    )
                else:
                    absolute_pattern = file_pattern
                matched_files = glob.glob(absolute_pattern, recursive=True)
                logger.debug(
                    f"Glob pattern {absolute_pattern} matched {len(matched_files)} files: {matched_files}"
                )
                expanded_files.extend(matched_files)
            else:
                # It's a direct file path
                logger.debug(f"Processing direct file path: {file_pattern}")
                # Check if the path is absolute, if not make it relative to current workspace
                if not os.path.isabs(file_pattern):
                    current_workspace = os.getcwd()
                    absolute_file_path = os.path.join(current_workspace, file_pattern)
                    logger.debug(
                        f"Converting relative path {file_pattern} to absolute: {absolute_file_path}"
                    )
                else:
        ...
        logger.info(
            f"Final expanded file list: {len(expanded_files)} files - {expanded_files}"
        )
        ...
        for file_path in expanded_files:
            try:
                logger.info(
                    f"Processing file {successful + failed + 1}/{len(expanded_files)}: {file_path}"
                )
                logger.debug(
                    f"About to call sync_markdown_to_page for file: {file_path} with space_key={space_key}, sync_mode={sync_mode_enum}, conflict_strategy={conflict_strategy_enum}, dry_run={dry_run}"
                )
                ...
                logger.debug(
                    f"Sync result for {file_path}: success={result.success}, message={result.message}, page_id={result.page_id}"
                )
                ...
                if result.success:
                    successful += 1
                    logger.info(
                        f"Successfully synced file: {file_path} -> page_id: {result.page_id}"
                    )
                else:
                    failed += 1
                    logger.warning(
                        f"Failed to sync file: {file_path} - {result.message}"
                    )
        ...
        logger.info(
            f"Batch sync completed: {successful}/{len(expanded_files)} files successful"
        )
        return json.dumps(response, indent=2, ensure_ascii=False)

...
class PageMatcher:
        ...
        try:
            ...
            if frontmatter and "confluence_page_id" in frontmatter:
                ...
                if exact_match:
                    ...
                    logger.warning(
                        f"Page ID {page_id} from frontmatter not found in space"
                    )

            # Filter pages by space
            space_pages = [
                p for p in existing_pages if p.get("space", {}).get("key") == space_key
            ]
            ...
            if fuzzy_match:
                logger.debug(
                    f"Found fuzzy title match: {title} -> {fuzzy_match['title']}"
                )
                return fuzzy_match
    ...
    def _find_page_by_id(
        self, page_id: str, pages: list[dict[str, Any]]
    ) -> dict[str, Any] | None:
    ...
    def detect_conflicts(
        ...
        try:
            ...
            remote_modified_check = (
                True  # Placeholder - implement based on your sync metadata
            )
    ...
    def suggest_page_hierarchy(
        self, file_path: str, base_directory: str, existing_pages: list[dict[str, Any]]
    ) -> str | None:
        """
        ...
        try:
            ...
            for page in existing_pages:
                ...
                if parent_dir.lower() in page_title or page_title in parent_dir.lower():
                    logger.debug(
                        f"Found potential parent page: {page['title']} for {file_path}"
                    )
                    return page.get("id")
    ...
    def validate_sync_compatibility(
        self, markdown_file: dict[str, Any], confluence_page: dict[str, Any]
    ) -> tuple[bool, list[str]]:
        """
        ...
        try:
            ...
            if (
                confluence_page.get("restrictions", {})
                .get("read", {})
                .get("restrictions", {})
                .get("user", {})
                .get("results")
            ...
            page_content = (
                confluence_page.get("body", {}).get("storage", {}).get("value", "")
            )
            if "ac:structured-macro" in page_content:
                ...
                if macro_count > 10:  # Arbitrary threshold
                    issues.append(
                        f"Page contains many macros ({macro_count}) that may not convert well"
                    )
            ...
            if content_size > 100000:  # 100KB threshold
                issues.append(
                    f"Page content is very large ({content_size} chars) and may have sync issues"
                )

...
def get_image_dimensions(data: bytes) -> tuple[int, int] | None:
    ...
    renders inline: PNG, GIF, JPEG, BMP, and WebP. Jira Cloud needs
    ``width``/``height`` on an inline ``mediaSingle`` node for the image to
...
def _webp_dimensions(data: bytes) -> tuple[int, int] | None:
    """Extract dimensions from the VP8/VP8L/VP8X chunk of a WebP file."""
    chunk = data[12:16]
...
def _jpeg_dimensions(data: bytes) -> tuple[int, int] | None:
    ...
    # SOF markers carry the frame size. SOF4 (0xC4), SOF8 (0xC8) and SOF12
    # (0xCC) are DHT/JPG/DAC tables rather than frame headers, so they are

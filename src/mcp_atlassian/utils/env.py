...
def is_env_falsy(env_var_name: str, default: str = "") -> bool:
    """Check if an environment variable is set to a falsy value.

    Considers 'false', '0', 'no', 'n', 'off' as falsy values (case-insensitive).
    ...
    Returns:
        True if the environment variable is set to a falsy value, False otherwise
    """
    return os.getenv(env_var_name, default).lower() in ("false", "0", "no", "n", "off")

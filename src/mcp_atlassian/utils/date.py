...
def parse_date(date_str: str | int | None) -> datetime | str | None:
    ...
    if isinstance(date_str, int) or date_str.isdigit():
        try:
            ...
            if -62135596800000 <= timestamp_ms <= 253402300799999:
                ...
                logger.warning(
                    f"Timestamp {timestamp_ms} is out of Python datetime range (year 1-9999). "
                    "Returning as string. This may occur with legacy Jira Server instances."
                )
    ...
    try:
        ...
        logger.warning(f"Failed to parse date string '{date_str}': {e}. Returning as-is.")
        return date_str

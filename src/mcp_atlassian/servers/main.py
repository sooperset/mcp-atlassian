...
class AtlassianMCP(FastMCP[MainAppContext]):
    ...
    async def _tools_by_name(self) -> dict[str, FastMCPTool]:
        """Return registered FastMCP tools keyed by their public tool name."""
        return {tool.name: tool for tool in await self.list_tools()}
    ...
    async def _list_tools_mcp(self) -> list[MCPTool]:
        # Filter tools based on enabled tools, read-only mode, and service state.
        ...
        all_tools = await self._tools_by_name()
        logger.debug(
    ...
    async def _call_tool_mcp(self, key: str, arguments: dict[str, Any]) -> Any:
        ...
        # hidden tool cannot be invoked directly by name.
        ctx = self._tool_filter_context()
        if ctx is not None:
            tool_obj = (await self._tools_by_name()).get(key)

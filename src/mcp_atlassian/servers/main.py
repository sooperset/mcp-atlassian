...
class AtlassianMCP(FastMCP[MainAppContext]):
    ...
    async def _list_tools_mcp(self) -> list[MCPTool]:
        ...
        # Check for header-based service availability (from PR #683)
        ...
        # Check for per-request configuration overrides (from HTTP headers - Issue #850)
        ...
        logger.debug(
            ...
            f"enabled_tools_filter={enabled_tools_filter}, header_services={header_based_services}"
        )
...
class UserTokenMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # Initialize per-request configuration overrides (for multi-user HTTP deployments - Issue #850)
        # Note: Authentication is handled by PR #683 via atlassian_service_headers
    ...
    def _process_config_headers(self, headers: dict, scope: Scope) -> None:
        ...
        read_only_mode = self._get_header_str(headers, b"x-atlassian-read-only-mode")
        if read_only_mode:
            scope["state"]["read_only_mode"] = read_only_mode
            logger.debug(
                f"UserTokenMiddleware: X-Atlassian-Read-Only-Mode header found: {read_only_mode}"
            )

        # Per-request filters
        jira_projects_filter = self._get_header_str(
            headers, b"x-atlassian-jira-projects-filter"
        )
        if jira_projects_filter:
            scope["state"]["jira_projects_filter"] = jira_projects_filter
            logger.debug(
                f"UserTokenMiddleware: X-Atlassian-Jira-Projects-Filter header found: {jira_projects_filter}"
            )

        confluence_spaces_filter = self._get_header_str(
            headers, b"x-atlassian-confluence-spaces-filter"
        )
        if confluence_spaces_filter:
            scope["state"]["confluence_spaces_filter"] = confluence_spaces_filter
            logger.debug(
                f"UserTokenMiddleware: X-Atlassian-Confluence-Spaces-Filter header found: {confluence_spaces_filter}"
            )

        # Per-request enabled tools
        enabled_tools = self._get_header_str(headers, b"x-atlassian-enabled-tools")
        if enabled_tools:
            scope["state"]["enabled_tools"] = enabled_tools
            logger.debug(
                f"UserTokenMiddleware: X-Atlassian-Enabled-Tools header found: {enabled_tools}"
            )

...
# Zephyr Squad (Jira Plugin) Tools


@jira_mcp.tool(
    tags={"jira", "zephyr-squad", "read"},
    annotations={"title": "Get Zephyr Squad Test Cycles", "readOnlyHint": True},
)
async def zephyr_squad_get_cycles(
    ctx: Context,
    project_id: Annotated[str, Field(description="Numeric project ID (not key)")],
    version_id: Annotated[
        str | None, Field(description="Optional version ID to filter cycles")
    ] = None,
) -> str:
    """
    Get Zephyr Squad test cycles for a project.

    Zephyr Squad is a test management plugin for Jira.
    Use the numeric project ID, not the project key.
    ...
    Returns:
        JSON string with test cycles data.
    """
    jira = await get_jira_fetcher(ctx)
    try:
        result = jira.get_zephyr_cycles(project_id=project_id, version_id=version_id)
        return json.dumps(result, indent=2, ensure_ascii=False)
    except Exception as e:
        logger.error(f"Error getting Zephyr cycles: {str(e)}")
        return json.dumps({"success": False, "error": str(e)}, indent=2)


@jira_mcp.tool(
    tags={"jira", "zephyr-squad", "write"},
    annotations={"title": "Create Zephyr Squad Test Cycle"},
)
@check_write_access
async def zephyr_squad_create_cycle(
    ctx: Context,
    project_id: Annotated[str, Field(description="Numeric project ID")],
    version_id: Annotated[str, Field(description="Version ID")],
    name: Annotated[str, Field(description="Cycle name")],
    description: Annotated[
        str | None, Field(description="Optional description")
    ] = None,
    start_date: Annotated[
        str | None, Field(description="Start date (format: dd/MMM/yy)")
    ] = None,
    end_date: Annotated[
        str | None, Field(description="End date (format: dd/MMM/yy)")
    ] = None,
) -> str:
    """
    ...
    Returns:
        JSON string with created cycle data.
    """
    jira = await get_jira_fetcher(ctx)
    try:
        result = jira.create_zephyr_cycle(
        ...
        logger.error(f"Error creating Zephyr cycle: {str(e)}")
        return json.dumps({"success": False, "error": str(e)}, indent=2)


@jira_mcp.tool(
    tags={"jira", "zephyr-squad", "read"},
    annotations={"title": "Get Zephyr Squad Test Executions", "readOnlyHint": True},
)
async def zephyr_squad_get_executions(
    ctx: Context,
    cycle_id: Annotated[
        str | None, Field(description="Optional cycle ID to filter by")
    ] = None,
    issue_id: Annotated[
        str | None, Field(description="Optional issue ID to filter by")
    ] = None,
    offset: Annotated[int, Field(description="Pagination offset")] = 0,
    limit: Annotated[int, Field(description="Number of results")] = 50,
) -> str:
    """
    Get Zephyr Squad test executions.
    ...
    Returns:
        JSON string with execution data.
    """
    jira = await get_jira_fetcher(ctx)
    try:
        result = jira.get_zephyr_executions(
            cycle_id=cycle_id, issue_id=issue_id, offset=offset, limit=limit
        )
        return json.dumps(result, indent=2, ensure_ascii=False)
    except Exception as e:
        logger.error(f"Error getting Zephyr executions: {str(e)}")
        return json.dumps({"success": False, "error": str(e)}, indent=2)


@jira_mcp.tool(
    tags={"jira", "zephyr-squad", "write"},
    annotations={"title": "Create Zephyr Squad Test Execution"},
)
@check_write_access
async def zephyr_squad_create_execution(
    ctx: Context,
    issue_id: Annotated[str, Field(description="Issue ID (test case)")],
    cycle_id: Annotated[str, Field(description="Cycle ID")],
    project_id: Annotated[str, Field(description="Project ID")],
    version_id: Annotated[str, Field(description="Version ID")],
    assignee_type: Annotated[
        str, Field(description="Assignee type (e.g., 'assignee', 'currentUser')")
    ] = "assignee",
    assignee: Annotated[
        str | None, Field(description="Optional assignee username")
    ] = None,
) -> str:
    """
    Create a Zephyr Squad test execution.
    ...
    Returns:
        JSON string with created execution data.
    """
    jira = await get_jira_fetcher(ctx)
    try:
        result = jira.create_zephyr_execution(
            issue_id=issue_id,
            cycle_id=cycle_id,
            project_id=project_id,
            version_id=version_id,
        ...
        logger.error(f"Error creating Zephyr execution: {str(e)}")
        return json.dumps({"success": False, "error": str(e)}, indent=2)


@jira_mcp.tool(
    tags={"jira", "zephyr-squad", "write"},
    annotations={"title": "Execute Zephyr Squad Test"},
)
@check_write_access
async def zephyr_squad_execute_test(
    ctx: Context,
    execution_id: Annotated[str, Field(description="Execution ID")],
    status: Annotated[
        str, Field(description="Status ID (e.g., '1' for Pass, '2' for Fail)")
    ],
    comment: Annotated[str | None, Field(description="Optional comment")] = None,
) -> str:
    """
    Execute a Zephyr Squad test (set its status).
    ...
    Returns:
        JSON string with updated execution data.
    """
    jira = await get_jira_fetcher(ctx)
    try:
        result = jira.execute_zephyr_test(
            execution_id=execution_id, status=status, comment=comment
        )
        return json.dumps(result, indent=2, ensure_ascii=False)
    except Exception as e:
        logger.error(f"Error executing Zephyr test: {str(e)}")
        return json.dumps({"success": False, "error": str(e)}, indent=2)


@jira_mcp.tool(
    tags={"jira", "zephyr-squad", "read"},
    annotations={"title": "Get Zephyr Squad Test Steps", "readOnlyHint": True},
)
async def zephyr_squad_get_test_steps(
    ctx: Context,
    issue_id: Annotated[str, Field(description="Issue ID of the test case")],
) -> str:
    """
    Get test steps for a Zephyr Squad test case.

    Args:
        ctx: The FastMCP context.
        issue_id: Issue ID of test case

    Returns:
        JSON string with test steps.
    """
    jira = await get_jira_fetcher(ctx)
    try:
        result = jira.get_zephyr_test_steps(issue_id=issue_id)
        return json.dumps(result, indent=2, ensure_ascii=False)
    except Exception as e:
        logger.error(f"Error getting Zephyr test steps: {str(e)}")
        return json.dumps({"success": False, "error": str(e)}, indent=2)

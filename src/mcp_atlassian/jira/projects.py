...
class ProjectsMixin(JiraClient, SearchOperationsProto):
    ...
    def get_priorities(self) -> list[dict[str, Any]]:
        ...
        data = self.jira.get(self.jira.resource_url("priority", api_version="3"))
    ...
    def get_resolutions(self) -> list[dict[str, Any]]:
        ...
        data = self.jira.get(self.jira.resource_url("resolution", api_version="3"))
        if not isinstance(data, list):
    ...
    def get_statuses(self) -> list[dict[str, Any]]:
        ...
        data = self.jira.get(self.jira.resource_url("status", api_version="3"))
        if not isinstance(data, list):
    ...
    def get_project_statuses(self, project_key: str) -> list[dict[str, Any]]:
        ...
        data = self.jira.get(
            self.jira.resource_url(
                f"project/{project_key}/statuses", api_version="3"
            )
        )
        if not isinstance(data, list):

...
class WorkAttributeMixin(JiraClient):
    ...
    def get_work_attribute_catalog(self) -> list[JiraWorkAttribute]:
        ...
        Tempo Core returns ``staticListValues`` inline on the attribute
        definition, so values are only requested separately for definitions
        that omit them. This avoids one redundant request per static-list
        attribute.
        ...
        for attribute in attributes:
            ...
            if (
                attribute.id > 0
                and type_value == "STATIC_LIST"
                and not attribute.static_list_values
            ):

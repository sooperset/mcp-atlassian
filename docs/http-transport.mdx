...
<Steps>
  ...
  <Step title="Configure Environment">
    ...
    # Option A (legacy global Cloud OAuth; single shared app):
    ...
    # Option B (service-scoped; required for mixed Cloud/DC or separate OAuth clients):
    #JIRA_OAUTH_INSTANCE_TYPE=cloud
    #JIRA_OAUTH_CLOUD_ID=your_jira_cloud_id_from_setup_wizard
    ...
    #CONFLUENCE_OAUTH_CLOUD_ID=your_confluence_cloud_id_from_setup_wizard
...
- Falls back to service-scoped `JIRA_OAUTH_CLOUD_ID` / `CONFLUENCE_OAUTH_CLOUD_ID` if configured
- Otherwise falls back to global `ATLASSIAN_OAUTH_CLOUD_ID`
</Note>

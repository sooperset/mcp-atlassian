...
| `worklog_attributes` | `string` | No | (Optional) JSON string of Tempo Worklog Attributes. Example: `{"_WorklogCategory_": {"value": "Bugfixing"}}`. Use `jira_get_work_attributes` to find available attribute keys and `jira_get_work_attribute_values` to find static-list values. Available only on Jira Server/Data Center with Tempo Timesheets installed. |

---

### Get Work Attributes

Get all Tempo Core Work Attribute definitions configured in Jira, including their keys, types, and static-list values.

<Note>Available only for Jira Server/Data Center with Tempo Timesheets plugin installed.</Note>
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `attribute_id` | `integer` | Yes | The Tempo Work Attribute ID (e.g., 45) |

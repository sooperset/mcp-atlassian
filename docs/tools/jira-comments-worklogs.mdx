...
| `media` | `string` | No | (Optional, Jira Cloud only) JSON array of images to embed inline in the comment body. Each entry is either `{"file_path":"screenshots/before.png"}` — a path inside the server's working directory — or `{"filename":"before.png","content_base64":"<b64>"}` for content the server cannot read from disk. Position each image in 'body' with Markdown image syntax targeting its index or filename, e.g. '![before](media:0)' or '![before](before.png)', to render text -> screenshot -> text -> screenshot; unreferenced images are appended at the end. Images are uploaded to the issue as attachments; if any step fails they are all deleted again. Cannot be combined with 'public'. |

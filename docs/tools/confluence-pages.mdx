...
| `convert_to_markdown` | `boolean` | No | Whether to convert page to markdown (true) or keep it in raw Confluence storage format (false). Raw storage can reveal macros (like dates) not visible in markdown, but CAUTION: using storage format significantly increases token usage in AI responses. |
**Example:**
...
| `convert_to_markdown` | `boolean` | No | Whether to convert page content to markdown (true) or keep it in raw Confluence storage format (false). Only relevant if include_content is true. |

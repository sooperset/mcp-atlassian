...
Get all key-value metadata stored on a Confluence page, or retrieve one property by key.
...
| `key` | `string` | No | A specific property key. Omit it to return all properties. |

---

### Set Content Property

Create or update key-value metadata on a Confluence page. Existing property versions are incremented automatically.
...
|-----------|------|----------|-------------|
| `page_id` | `string` | Yes | The ID of the Confluence page |
| `key` | `string` | Yes | The property key to create or update |
| `value` | `string` | Yes | The property value encoded as JSON, such as `"full-width"` or `{"version": 2}` |

**Example:**

```json
{"page_id": "12345678", "key": "editor", "value": "{\"version\": 2}"}
```

...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `comment_id` | `string` | Yes | The ID of the footer or inline comment to delete |
**Example:**
...
<Tip>
Works for both footer and inline comments. Get comment IDs from `confluence_get_comments` or `confluence_get_inline_comments`. Deletion is permanent.
</Tip>

...
|-----------|------|----------|-------------|
| `board_name` | `object` | No | Name of the board (fuzzy search). |
| `project_key` | `object` | No | Project key. |
| `board_type` | `object` | No | Board type ('scrum' or 'kanban'). |
...
| `state` | `object` | No | Sprint state ('active', 'future', 'closed'). If None, returns all sprints. |
| `start_at` | `integer` | No | Starting index for pagination (0-based) |
| `limit` | `integer` | No | Maximum number of results (1-50) |

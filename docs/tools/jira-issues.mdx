...
| `delete_attachments` | `string` | No | (Optional) JSON array or comma-separated list of attachment IDs to delete from the issue. IDs come from the issue's attachment metadata (e.g. jira_get_issue). Example: '10001,10002'. |
| `delete_comments` | `string` | No | (Optional) JSON array or comma-separated list of comment IDs to delete from the issue. IDs come from the issue's comments (e.g. jira_get_issue with comments included). |

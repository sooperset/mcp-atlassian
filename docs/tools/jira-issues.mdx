...
`fields` is optional when you only need to transition, comment, or log work. If `transition` and `comment` are both provided without `comment_visibility`, the comment is sent with the transition.

...
| `projects_filter` | `object` | No | Comma-separated list of project keys to filter by. |
| `expand` | `object` | No | Optional fields to expand. |
| `page_token` | `object` | No | Pagination token from a previous search result (Cloud only). |
...
| `context_id` | `object` | No | Field context ID (Cloud only, auto-resolved if omitted). |
| `project_key` | `object` | No | Project key (required for Server/DC). |
| `issue_type` | `object` | No | Issue type name (required for Server/DC). |
| `contains` | `object` | No | Case-insensitive substring filter on option values. |
| `return_limit` | `object` | No | Cap on number of results after filtering. |
...
| `current_project_ids` | `object` | No | Comma-separated project IDs to exclude. |

...
Use a `*_COMMAND` variable instead of storing its token directly. MCP Atlassian
runs the command on the first tool call for that service, trims its standard
output, and reuses the result for the rest of the process. Startup and tool
listing don't run the command.

```bash
JIRA_URL=https://company.atlassian.net
JIRA_USERNAME=user@company.com
JIRA_API_TOKEN_COMMAND='gopass show -o my/jira/token'
```

| Variable | Description |
|----------|-------------|
| `JIRA_API_TOKEN_COMMAND` | Command that returns a Jira API token (Cloud or Server/DC basic auth) |
| `JIRA_PERSONAL_TOKEN_COMMAND` | Command that returns a Jira PAT (Server/DC) |
| `CONFLUENCE_API_TOKEN_COMMAND` | Command that returns a Confluence API token (Cloud or Server/DC basic auth) |
| `CONFLUENCE_PERSONAL_TOKEN_COMMAND` | Command that returns a Confluence PAT (Server/DC) |
| `CREDENTIAL_COMMAND_TIMEOUT` | Positive integer timeout in seconds (default: `30`) |

The plain token variable takes precedence over its `*_COMMAND` counterpart.
API token commands still require the matching `*_USERNAME`. Commands run
without an implicit shell; invoke one explicitly (for example, `sh -c '...'`)
if you need pipes or redirects. Standard input is closed, so a helper can't
read from it — this keeps it from consuming the MCP stdio transport.

On Server/Data Center, `*_PERSONAL_TOKEN_COMMAND` outranks static
username/API-token basic auth, matching the precedence a plain
`*_PERSONAL_TOKEN` would have. If the command then fails, the eagerly loaded
configuration is used instead, so a locked secret store can't take down a
server that has other working credentials. On Cloud, OAuth still outranks
`*_API_TOKEN_COMMAND`, and the command is never run in that case.

<Note>
On Windows the command string is passed to `CreateProcess` unparsed, so use
double quotes (`op read "op://Vault/Jira PAT/credential"`) — POSIX single
quotes don't quote there. `.bat`, `.cmd`, and PowerShell helpers need an
explicit `cmd /c` or `powershell -NoProfile -Command` prefix. On POSIX,
arguments follow `shlex` rules.
</Note>

...
| `MCP_ATLASSIAN_VALIDATION_CACHE_TTL` | Seconds to reuse successful multi-user credential validation (default `300`; `0` disables) |
| `MCP_ATLASSIAN_VALIDATION_CACHE_MAXSIZE` | Maximum number of successful validation results to retain (default `100`; `0` disables) |
...
The validation cache stores only successful validation data. It is scoped by
service, target URL, tenant, and a digest of the authentication material; raw
credentials and fetchers are not cached. If an upstream credential is revoked,
the server may continue to accept it for up to the configured TTL. Set the TTL
or maximum size to `0` when immediate revocation visibility is more important
than reducing repeated validation calls.

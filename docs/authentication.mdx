...
Atlassian identity provider, not a dynamically registered MCP client. DCR rejects a
client `redirect_uri` that targets this OAuth proxy callback with
`invalid_redirect_uri`, preventing a callback loop.
</Warning>

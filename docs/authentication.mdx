...
1. Enable multi-user mode:
   ...
   The legacy `ATLASSIAN_OAUTH_ENABLE=true` flag remains a backwards-compatible
   alias for this mode.

2. Each client sends one of the following headers per request:
   - `Authorization: Basic <base64(email:api_token)>` (Cloud)
   - `Authorization: Bearer <user_oauth_token>` (Cloud)
   - `Authorization: Token <user_personal_access_token>` (Server/Data Center)
   - Optional: `X-Atlassian-Cloud-Id: <user_cloud_id>` for OAuth multi-cloud

<Note>
For SSRF protection, multi-user mode ignores `X-Atlassian-Jira-Url` and
`X-Atlassian-Confluence-Url` headers; the server enforces the URLs configured
at startup. Requires HTTP transport (sse / streamable-http).
</Note>

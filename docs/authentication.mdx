...
<Warning>
`ATLASSIAN_OAUTH_SCOPE` must be non-empty when the proxy is enabled. The service
does not expose OAuth proxy or DCR routes without an explicit scope boundary.
</Warning>

...
class TestGetAvailableServices:
    ...
    def test_oauth_enable_without_urls(self, caplog):
        ...
        with MockEnvironment.clean_env():
            ...
            assert_log_contains(
                caplog, "INFO", "Using Confluence minimal OAuth configuration - expecting user-provided tokens via headers"
            )
            assert_log_contains(
                caplog, "INFO", "Using Jira minimal OAuth configuration - expecting user-provided tokens via headers"
            )

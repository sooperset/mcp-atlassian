...
from tests.utils.mocks import MockOAuthServer, MockEnvironment
...
class TestRedirectUriParsing:
    ...
    @pytest.mark.parametrize("redirect_uri,expected_hostname,expected_port", [
...
class TestOAuthFlow:
    ...
    def test_run_oauth_flow_success_localhost(self):
        ...
        with MockOAuthServer.mock_oauth_flow() as mocks:
            with patch("mcp_atlassian.utils.oauth_setup.OAuthConfig") as mock_oauth_config, \
                 patch("mcp_atlassian.utils.oauth_setup.wait_for_callback") as mock_wait, \
                 patch("mcp_atlassian.utils.oauth_setup.start_callback_server") as mock_start_server:
    ...
    def test_run_oauth_flow_server_start_failure(self):
        ...
        with MockOAuthServer.mock_oauth_flow() as mocks:
            with patch("mcp_atlassian.utils.oauth_setup.start_callback_server") as mock_start_server:
    ...
    @pytest.mark.parametrize("failure_condition,expected_result", [
        ("timeout", False),
...
class TestInteractiveSetup(BaseAuthTest):
    ...
    def test_run_oauth_setup_with_env_vars(self):
        ...
        with MockEnvironment.oauth_env() as env_vars:
            with patch("builtins.input", side_effect=["", "", "", ""]), \
                 patch("mcp_atlassian.utils.oauth_setup.run_oauth_flow", return_value=True) as mock_flow:
                ...
                assert_config_contains(vars(args), 
    ...
    @pytest.mark.parametrize("input_values,expected_result", [
        (["user-client-id", "user-secret", "http://localhost:9000/callback", "read:jira-work"], 0),
    ...
    def test_run_oauth_setup_flow_failure(self):
        ...
        with MockEnvironment.clean_env():
            with patch("builtins.input", side_effect=["client-id", "client-secret", "", ""]), \
                 patch("mcp_atlassian.utils.oauth_setup.run_oauth_flow", return_value=False):

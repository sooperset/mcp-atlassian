...
def read_only_test_confluence_mcp(mock_confluence_fetcher):
    ...
    from src.mcp_atlassian.servers.confluence import (
        add_label, create_page, delete_page, get_comments, get_labels, get_page,
        get_page_ancestors, get_page_children, search, update_page,
    )
    test_mcp.tool()(search)
...
async def test_search(client, mock_confluence_fetcher):
    ...
    # Check if it defaulted to siteSearch
...
async def test_add_label(client, mock_confluence_fetcher):
    ...
    response = await client.call_tool("add_label", {"page_id": "123456", "name": "new-label"})
    mock_confluence_fetcher.add_page_label.assert_called_once_with("123456", "new-label")
    result_data = json.loads(response[0].text)
    assert isinstance(result_data, list)

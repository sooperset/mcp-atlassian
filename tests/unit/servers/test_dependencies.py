...
def config_factory():
    ...
    class ConfigFactory:
        ...
        def create_jira_config(auth_type="basic", **overrides):
            ...
            if auth_type == "basic":
                defaults.update({"username": "test_username", "api_token": "test_token"})
        ...
        def create_app_context(jira_config=None, confluence_config=None, **overrides):
            ...
            defaults = {
                ...
                "full_confluence_config": confluence_config or ConfigFactory.create_confluence_config(),
                "read_only": False,
...
def _assert_config_attributes(config, expected_type, expected_auth_type, expected_token=None):
...
class TestCreateUserConfigForFetcher:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("oauth", "oauth_access_token", "OAuth access token missing in credentials"),
    ...
    def test_missing_oauth_config(self, config_factory):
        ...
        base_config = config_factory.create_jira_config(auth_type="basic")  # No OAuth config
...
def _create_mock_fetcher(fetcher_class, validation_return=None, validation_error=None):
    ...
    if fetcher_class == JiraFetcher:
        if validation_error:
            ...
            mock_fetcher.get_current_user_account_id.return_value = validation_return or "test-account-id"
...
class TestGetJiraFetcher:
    ...
    async def test_user_specific_fetcher_creation(
        ...
        confluence_config = config_factory.create_confluence_config(auth_type=scenario["auth_type"])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("missing_lifespan_context", "Jira global configuration.*is not available from lifespan context"),
        ],
    )
    @patch("mcp_atlassian.servers.dependencies.get_http_request")
    @patch("mcp_atlassian.servers.dependencies.JiraFetcher")
...
class TestGetConfluenceFetcher:
    ...
    async def test_cached_fetcher_returned(
        self, mock_confluence_fetcher_class, mock_get_http_request, mock_context, mock_request
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("missing_lifespan_context", "Confluence global configuration.*is not available from lifespan context"),
        ],
    )
    @patch("mcp_atlassian.servers.dependencies.get_http_request")
    @patch("mcp_atlassian.servers.dependencies.ConfluenceFetcher")

...
@pytest.mark.anyio
async def test_add_comment_public_false_without_visibility_keeps_internal_routing(
    jira_client, mock_jira_fetcher
):
    """Test add_comment preserves explicit public=false for JSM internal notes."""
    await jira_client.call_tool(
        "jira_add_comment",
        {
            "issue_key": "TEST-123",
    ...
    mock_jira_fetcher.add_comment.assert_called_once_with(
        "TEST-123", "Internal note", None, public=False
    )

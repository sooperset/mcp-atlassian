...
class TestMCPProtocolIntegration:
    ...
    async def mock_jira_fetcher(self):
        ...
        fetcher.search_issues.return_value = {
            "issues": [JiraIssueFactory.create("TEST-1"), JiraIssueFactory.create("TEST-2")],
            "total": 2
        }
        return fetcher
    ...
    async def mock_confluence_fetcher(self):
        ...
        fetcher.search_pages.return_value = {
            "results": [ConfluencePageFactory.create("123"), ConfluencePageFactory.create("456")],
            "size": 2
        }
        return fetcher
    ...
    async def test_tool_discovery_with_full_configuration(self, atlassian_mcp_server, mock_jira_config, mock_confluence_config):
        """Test tool discovery when both Jira and Confluence are fully configured."""
        with MockEnvironment.basic_auth_env():
            # Mock the configuration loading
            with patch("mcp_atlassian.jira.config.JiraConfig.from_env", return_value=mock_jira_config), \
                 patch("mcp_atlassian.confluence.config.ConfluenceConfig.from_env", return_value=mock_confluence_config):
                ...
                async def mock_get_tools():
                    ...
                    for tool_name in ["jira_get_issue", "jira_create_issue", "jira_search_issues"]:
                        tool = MagicMock(spec=FastMCPTool)
                        tool.tags = {"jira", "read"} if "get" in tool_name or "search" in tool_name else {"jira", "write"}
                    ...
                    for tool_name in ["confluence_get_page", "confluence_create_page"]:
                        tool = MagicMock(spec=FastMCPTool)
                        tool.tags = {"confluence", "read"} if "get" in tool_name else {"confluence", "write"}
                        tool.to_mcp_tool.return_value = MCPTool(
                            name=tool_name,
                            description=f"Tool {tool_name}",
                            inputSchema={"type": "object", "properties": {}}
    ...
    async def test_tool_filtering_read_only_mode(self, atlassian_mcp_server, mock_jira_config, mock_confluence_config):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            async def mock_get_tools():
                ...
                read_tools = ["jira_get_issue", "jira_search_issues", "confluence_get_page"]
                write_tools = ["jira_create_issue", "jira_update_issue", "confluence_create_page"]
                
                for tool_name in read_tools:
                    tool = MagicMock(spec=FastMCPTool)
                    tool.tags = {"jira", "read"} if "jira" in tool_name else {"confluence", "read"}
                    tool.to_mcp_tool.return_value = MCPTool(
                        name=tool_name,
                        description=f"Tool {tool_name}",
                        inputSchema={"type": "object", "properties": {}}
                ...
                for tool_name in write_tools:
                    tool = MagicMock(spec=FastMCPTool)
                    tool.tags = {"jira", "write"} if "jira" in tool_name else {"confluence", "write"}
                    tool.to_mcp_tool.return_value = MCPTool(
                        name=tool_name,
                        description=f"Tool {tool_name}",
                        inputSchema={"type": "object", "properties": {}}
    ...
    async def test_tool_filtering_with_enabled_tools(self, atlassian_mcp_server, mock_jira_config, mock_confluence_config):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            async def mock_get_tools():
                tools = {}
                all_tools = ["jira_get_issue", "jira_create_issue", "jira_search_issues", 
    ...
    async def test_tool_filtering_service_not_configured(self, atlassian_mcp_server):
        ...
        with MockEnvironment.clean_env():
            ...
            async def mock_get_tools():
                tools = {}
                all_tools = ["jira_get_issue", "jira_create_issue", "confluence_get_page", "confluence_create_page"]
                
                for tool_name in all_tools:
                    tool = MagicMock(spec=FastMCPTool)
                    if "jira" in tool_name:
                        tool.tags = {"jira", "read"} if "get" in tool_name else {"jira", "write"}
                    else:
                        tool.tags = {"confluence", "read"} if "get" in tool_name else {"confluence", "write"}
                    tool.to_mcp_tool.return_value = MCPTool(
                        name=tool_name,
    ...
    async def test_middleware_invalid_auth_header(self):
        ...
        request.url.path = "/mcp"
        request.method = "POST"
        request.headers = {"Authorization": "Basic dXNlcjpwYXNz"}  # Basic auth not supported
        ...
        assert "Only 'Bearer <OAuthToken>' or 'Token <PAT>' types are supported" in body["error"]
    ...
    async def test_concurrent_tool_execution(self, atlassian_mcp_server, mock_jira_config, mock_confluence_config):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            assert execution_order.index("confluence_get_page_123_end") < execution_order.index("jira_get_issue_TEST-1_end")
            assert execution_order.index("confluence_get_page_456_end") < execution_order.index("jira_get_issue_TEST-2_end")
    ...
    async def test_lifespan_context_initialization(self):
        ...
        with MockEnvironment.basic_auth_env():
            with patch("mcp_atlassian.jira.config.JiraConfig.from_env") as mock_jira_config, \
                 patch("mcp_atlassian.confluence.config.ConfluenceConfig.from_env") as mock_conf_config:
    ...
    async def test_lifespan_with_enabled_tools(self):
        ...
        with MockEnvironment.basic_auth_env():
            with patch.dict(os.environ, {"ENABLED_TOOLS": "jira_get_issue,jira_search_issues,confluence_get_page"}):
                with patch("mcp_atlassian.jira.config.JiraConfig.from_env") as mock_jira_config:
                    # Configure mock
                    jira_config = MagicMock()
                    jira_config.is_auth_configured.return_value = True
                    ...
                    async with main_lifespan(app) as context:
                        app_context = context["app_lifespan_context"]
                        assert app_context.enabled_tools == ["jira_get_issue", "jira_search_issues", "confluence_get_page"]
    ...
    async def test_combined_filtering_scenarios(self, atlassian_mcp_server, mock_jira_config):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            app_context = MainAppContext(
                ...
                enabled_tools=["jira_get_issue", "jira_create_issue", "confluence_get_page"]  # Mix of tools
            )
            ...
            async def mock_get_tools():
                ...
                tool_configs = [
                    ...
                    ("jira_search_issues", {"jira", "read"}),  # Excluded by enabled_tools
                    ("confluence_get_page", {"confluence", "read"}),  # Excluded by service not configured
                ]

...
class TestRealJiraAPI(BaseAuthTest):
    ...
    def test_complete_issue_lifecycle(self, jira_client, test_project_key, created_issues):
    ...
    def test_attachment_upload_download(self, jira_client, test_project_key, created_issues, tmp_path):
    ...
    def test_jql_search_with_pagination(self, jira_client, test_project_key):
        ...
        # First page
        results_page1 = jira_client.search_issues(
            jql=jql,
            start_at=0,
            max_results=2
        )
...
class TestRealConfluenceAPI(BaseAuthTest):
    ...
    def test_cql_search(self, confluence_client, test_space_key):
        ...
        results = confluence_client.search_content(
            cql=cql,
            limit=5
        )
    ...
    def test_attachment_handling(self, confluence_client, test_space_key, created_pages, tmp_path):
    ...
    def test_large_content_handling(self, confluence_client, test_space_key, created_pages):
...
class TestCrossServiceIntegration:
    ...
    def test_jira_confluence_linking(self, jira_client, confluence_client, test_project_key, test_space_key, created_issues, created_pages):
        ...
        try:
            ...
            confluence_url = f"{confluence_client.config.url}/pages/viewpage.action?pageId={page.id}"

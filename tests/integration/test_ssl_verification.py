...
class TestSSLVerificationEnhanced(BaseAuthTest):
    ...
    def test_ssl_verification_with_custom_ca_bundle(self):
        ...
        with MockEnvironment.basic_auth_env() as env_vars:
            ...
            # For Jira
            jira_config = JiraConfig.from_env()
            assert jira_config.ssl_verify == custom_ca_path
            
            # For Confluence
            confluence_config = ConfluenceConfig.from_env()
            assert confluence_config.ssl_verify == custom_ca_path
    ...
    def test_ssl_configuration_persistence_across_requests(self):
        ...
        # Same adapter instance should be present
        assert session.adapters.get("https://test.atlassian.net") is adapter

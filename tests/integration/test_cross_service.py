...
class TestSharedAuthentication:
    ...
    def test_oauth_shared_configuration(self):
        ...
        with MockEnvironment.oauth_env() as env:
            ...
            assert jira_config.oauth_config.client_id == confluence_config.oauth_config.client_id
            assert jira_config.oauth_config.client_secret == confluence_config.oauth_config.client_secret
            assert jira_config.oauth_config.cloud_id == confluence_config.oauth_config.cloud_id
            assert jira_config.oauth_config.scope == confluence_config.oauth_config.scope
    ...
    async def test_authentication_context_in_request(self):
        ...
        with patch("mcp_atlassian.servers.dependencies.get_http_request", return_value=request):
            ...
            with patch("mcp_atlassian.jira.JiraFetcher") as mock_jira_fetcher, \
                 patch("mcp_atlassian.confluence.ConfluenceFetcher") as mock_confluence_fetcher:
...
class TestCrossServiceErrorHandling:
    ...
    async def test_confluence_failure_does_not_affect_jira(self):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            with patch("mcp_atlassian.confluence.config.ConfluenceConfig.from_env") as mock_conf_config:
    ...
    def test_ssl_configuration_shared(self):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            with patch.dict(os.environ, {
                ...
                configure_ssl_verification("Jira", jira_config.url, jira_session, ssl_verify=False)
                configure_ssl_verification("Confluence", confluence_config.url, confluence_session, ssl_verify=False)
...
class TestConcurrentServiceInitialization:
    ...
    async def test_concurrent_service_startup(self):
        ...
        with MockEnvironment.basic_auth_env():
            ...
            with patch("mcp_atlassian.jira.config.JiraConfig.from_env", side_effect=mock_jira_init), \
                 patch("mcp_atlassian.confluence.config.ConfluenceConfig.from_env", side_effect=mock_confluence_init):
...
class TestServiceAvailabilityDetection:
    ...
    def test_detect_only_jira_configured(self):
        ...
        }, clear=True):  # Clear environment to ensure isolation
            services = get_available_services()
...
class TestCrossServiceDataSharing:
    ...
    def test_shared_user_mentions(self):
        ...
        confluence_mention = f'<ac:link><ri:user ri:account-id="{user_account_id}" /></ac:link>'
        
        # Create content with mentions
        jira_comment = {
            "body": f"Hey {jira_mention}, please review this issue."
        }
